...
6. **Cluster-set review.** Review titles, stance purity, period or episode scope, source role, and granularity from representative members. Return one decision, an empty `issues` list when passed, one reason, and title-claim support; scripts own counts, range status, fingerprints, and mechanical confirmations. A normal period usually has 9–16 one-level clusters, but this is a review range, never a quota.
7. **Final excerpt review.** This is the single item-level semantic pass after clustering. Read `excerpt_segments` in numeric order and select segment indexes for aspect and stance evidence; do not copy text when a candidate works, and normal `keep` needs no reason. Scripts resolve exact evidence and already own target anchors, work-conflict markers, length, markup, and verbatim positions. Fill target or work-consistency fields only when explicitly flagged. Re-excerpt or reassign a repairable item before dropping it. Prefer 70–150 cleaned characters with a complete judgment and concrete supporting detail.
