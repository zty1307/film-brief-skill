...
- `REVIEW_REQUIRED`: copy the stated template to `required_file`, preserve `_workflow`, and complete every record from all `input_files`. The files are bounded chunks; process them in filename order. Use `full_input_file` only for auditing and use `full_source_file` only when an item needs more context.
...
2. **Source review.** Review only the generated compact queue. Decide `retain_core`, `retain_consensus`, or `exclude`; give a source-specific reason and choose `evidence_candidate_index` whenever possible. Exact text copies share one representative decision. For variety, retain the latest episode, a genuinely prominent new topic from the immediately previous episode, or current program-level discussion.
...
5. **Assignment repair.** For the residual queue, align target work, aspect, local stance, and verbatim passage. Use the cumulative `cluster_overrides.json`; an item still present after an override means that override failed and must change. A valid opinion should move to a fitting cluster or a newly evidenced cluster. Use `__exclude__` only for evidenced cross-work mismatch, out-of-scope text, no reportable viewpoint, abusive noise, pure promotion, or corrupt duplication.
6. **Cluster-set review.** Review titles, stance purity, period or episode scope, source role, and granularity from representative members. Every title clause needs at least one supporting member. A normal period usually has 9–16 one-level clusters, but this is a review range, never a quota. Do not force merges, splits, or deletions to hit the range.
7. **Final excerpt review.** This is the single item-level semantic pass after clustering. Process all chunk files. Judge the final cleaned excerpt itself for target, aspect, stance, work consistency, and self-contained meaning. Re-excerpt or reassign a repairable item before dropping it. Use one or two ordered, non-overlapping verbatim spans; never paraphrase. Prefer 70–150 cleaned characters with a complete judgment and concrete supporting detail. Shorter text needs a source-specific exception and specific evidence; generic reactions such as “好看”“封神”“笑点拉满” are insufficient.
...
Scripts own extraction, stable IDs, deterministic cleanup, hard AI-draft patterns, reviewed media lookup, safe link checks, exact-copy handling, scoring, fingerprints, source-position checks, HTML generation, and verification. AI owns ambiguous source relevance, irony and mixed stance, current-period cluster discovery, residual assignment, cluster-set judgment, and final excerpt semantics.
