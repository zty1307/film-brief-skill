...
7. **Final excerpt review.** This is the single item-level semantic pass after clustering. Process only the current chunk. For a normal item, judge its aspect, local stance, and self-contained meaning; scripts already own target anchors, work-conflict markers, length, markup, and verbatim positions. Fill target or work-consistency fields only when the input explicitly flags them. Re-excerpt or reassign a repairable item before dropping it. Use one or two ordered, non-overlapping verbatim spans; never paraphrase. Prefer 70–150 cleaned characters with a complete judgment and concrete supporting detail. Shorter text needs a source-specific exception and specific evidence; generic reactions such as “好看”“封神”“笑点拉满” are insufficient.
