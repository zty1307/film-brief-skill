...
Target a complete run within 30 minutes on an ordinary model. Preserve usable evidence and basic work/stance correctness; do not pursue perfect wording or one-to-one agreement with a finished report. Read only each current packet, submit once, and repair only reported errors. Do not generate stage essays or extra audit narratives. This is a runtime target to measure, not a claim that every model or dataset will meet it.
...
Failed final submissions carry field-level submission_errors directly in the current input_file. Correct those fields; do not reverse-engineer validators. Lexical hints can be corrected through an explicit semantic reason and never require deleting an otherwise usable source.
...
When one action fails twice with the same input fingerprint, the controller stops further retries. Read the latest failure file and change only the reported review or input; repeated `advance` calls cannot repair unchanged data.
...
2. **Source review.** Review only the generated compact queue. Decide `retain_core`, `retain_consensus`, or `exclude`, and choose `evidence_candidate_index` whenever possible. Retained sources need no repeated prose reason; when excluding for the displayed auto_reason, the controller reuses it after your explicit exclude decision; write a new reason only when your judgment differs. Never agree without reading. Exact text copies share one representative decision. For variety, retain the latest episode, a genuinely prominent new topic from the immediately previous episode, or current program-level discussion.
3. **Link and copy checks.** The controller checks links only for sources still eligible after content review and reuses recent definitive results from the project-local cache. Delete only a 404 or 410 reproduced by two GET requests. Login walls, 401/403/429, anti-bot responses, 5xx, DNS failures, and timeouts are indeterminate and are not cached as proof. Exact and high-confidence same-copy families are merged deterministically. Medium-similarity pairs remain independent by default, because similar viewpoints from different authors are useful evidence.
4. **Cluster discovery.** Read every file listed in `input_files` under `cluster_discovery_chunks` and derive current-period positive, objective, and negative clusters. The controller normally compresses all 180 stance- and channel-stratified passages into one handoff file; it does not reduce discovery coverage. All retained sources are still assigned and coverage-checked later. Each non-background cluster needs one to four aspect-bearing `required_any` terms; work names, person names, “剧情”, “热度”, or generic praise cannot be the routing basis. If many sources fail aspect alignment, the controller returns `cluster_definition_repair`: repair missing viewpoints or synonym terms before writing individual overrides.
...
6. **Cluster-set review.** Review titles, stance purity, period or episode scope, source role, and granularity from representative members. Return one decision, an empty `issues` list when passed, and one reason; scripts own counts, range status, fingerprints, and mechanical confirmations. Objective clusters may contain neutral facts, observations, or neutral summaries of opinion distribution; individual positive and negative evaluations belong in their matching stance clusters. A normal period usually has 9–16 one-level clusters, but this is a review range, never a quota.
7. **Final excerpt review.** The script first tries the ranked extractive candidates and automatically accepts only complete 70–150-character passages where one local segment contains both a concrete aspect and one unambiguous stance. Separate sentences may still form a valid excerpt, but AI must confirm their semantic relation. Broad retrieval words such as 群像、竞争、特效、质感, mixed, contrastive, short, promotional, cross-work, target-ambiguous and otherwise uncertain items remain in the AI `input_file`; do not reopen `excerpt_semantic_auto_accepted.jsonl`. For AI items, ordinary submissions need only decision=keep and evidence_candidate_index. Keep declares that you have read and confirmed the target, core aspect, local stance, and completeness; the controller fills the matching fingerprint, stance and confirmation fields. Use separate aspect/stance indexes only when necessary. Blank decisions never pass. Existing verbose reviews remain supported. A prefilled reason is needed only for ambiguity such as cross-work attribution or a disputed lexical hint; one concrete sentence is enough. When `cluster_claim_review_required` is true, confirm the core viewpoint. Each sample need not cover every parallel detail in the title; narrow an over-specific title rather than discard valid opinions. A roster, material announcement, brand action, or generic mention of a broad term does not support a narrower claim. When target review is flagged, confirm that the anchor and current opinion refer to the same work; a work name appearing only in a tag, roster, or roundup does not pass. Re-excerpt or reassign a repairable item before dropping it. No cluster is dropped because it has only one or two samples, and unchanged item fingerprints keep their completed reviews after a targeted repair.
...
- Short video without usable ASR is excluded from text evidence. Pure schedules, cast profiles, plot summaries, giveaways, ticketing, QR codes, and campaign instructions cannot support a viewpoint. A source containing a valid opinion may remain, but its displayed excerpt must omit purchase, lottery, follow, and participation instructions.
