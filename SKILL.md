...
7. **Final excerpt review.** The controller first returns any deterministic excerpt-format failures for targeted re-excerpting, then starts the single item-level semantic pass. Read `excerpt_segments` in numeric order and select segment indexes for aspect and stance evidence; do not copy text when a candidate works, and normal `keep` needs no reason. Scripts resolve exact evidence and already own target anchors, work-conflict markers, length, markup, and verbatim positions. Fill target or work-consistency fields only when explicitly flagged. Short exceptions require concrete actions, dialogue, scenes, data, or causal analysis; generic praise is dropped or re-excerpted. Re-excerpt or reassign a repairable item before dropping it. Unchanged item fingerprints keep their completed reviews after a targeted excerpt repair.
