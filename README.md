...
当前开发版本：v1.1.9。仓库包含完整的 Skill 说明、执行脚本、复核契约、媒体主体库和离线工作台模板，可独立安装和运行。
...
较大的来源复核会自动拆成每批最多60条；最终摘录复核每批最多80条且受90KB体积上限约束。控制器每轮只发出下一片未完成内容，并由脚本账本累计答案。来源输入不重复携带全文回查说明和脚本诊断量；最终摘录按编号选择证据，脚本自动回填逐字原文。多轮 `cluster_overrides.json` 同样由控制器累计。
