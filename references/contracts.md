...
`decision` 仅允许 `retain_core`、`retain_consensus`、`exclude`。`prepare` 已在队列中提供逐字候选片段和字符位置；优先复制其中的 `[start,end]` 到 `evidence_position`，脚本会安全提取完整 Unicode 原文，避免手工漏掉 emoji、空格或中间句。也可使用连续逐字 `evidence`。来源复核文件必须覆盖 `source_review_queue.jsonl` 的全部项目；中间运行可以暂时沿用自动结论，但只要 `source_review_queue.unresolved.jsonl` 非空，最终验证就不能 PASS。低于 `quality_floor` 但已经具备目标、判断和依据的来源进入 `retain_consensus`，质量分只影响排序。
...
每个批次必须至少一个簇；ID在批次内唯一；`stance` 必须是 `positive`、`objective`、`negative` 之一；标题应是能直接理解的完整报告体观点句，需出现“肯定、认可、关注、讨论、质疑、批评、担忧、认为、指出”等明确谓语；`记录` 只用于 `data_note`。“演员表现”“剧情张力”一类短标签不合格。负向簇填写 `negative_cues`，客观舆情概况可设 `objective_meta: true`。不要求每个批次三种立场齐全，也不要求每簇达到任何样本数量。簇的集合质量、信息层级和标题证据在下一份独立审查文件中确认，避免把旧审查字段残留在定义里。
...
页面展示前会执行 `remove_social_markup_v1`：仅删除话题标签、账号标记、平台表情、链接和共创视频等非观点标记，并在 `excerptProvenance.displayNormalization` 中记录版本。`fragments/positions` 仍保存未经改写的原文，展示文字必须能由这些原文片段按该规则复算。
