...
该文件只审核最终展示文字。控制器先要求修复 `excerpt_preflight_failures.json` 中的确定性格式错误，再每轮给出下一批最多80条，同时把单批文件限制在90KB以内。每条将清洁摘录无损拆成按顺序编号的 `excerpt_segments`；无风险常规项不重复携带前后文，只有短摘录、促销、对象或跨作品风险项才提供必要上下文。完整来源只在需要重截、转簇或核对时按 `source_id` 回查。
